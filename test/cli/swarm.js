...
describe("swarm", () => {
  ...
  describe("daemon on (through http-api)", function() {
    ...
    it("disconnect", async () => {
      ...
      expect(out).to.eql(`disconnect ${bMultiaddr} success\n`);
    });
  ...
  describe("handlers", () => {
    ...
    describe("addrs", () => {
      ...
      it("should return addresses for all peers", done => {
        ...
        sinon
          .stub(peerInfo.multiaddrs, "_multiaddrs")
          .value([
            multiaddr("/ip4/127.0.0.1/tcp/4001"),
            multiaddr(
              `/ip4/127.0.0.1/tcp/4001/ws/ipfs/${peerInfo.id.toB58String()}`
            )
          ]);

        sinon.stub(ipfs.swarm, "addrs").returns(Promise.resolve([peerInfo]));
