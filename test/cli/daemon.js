...
describe("daemon", () => {
  ...
  it("should allow bind to multiple addresses for API and Gateway", async function() {
    ...
    const apiAddrs = ["/ip4/127.0.0.1/tcp/0", "/dns4/localhost/tcp/0"];

    const gatewayAddrs = ["/ip4/127.0.0.1/tcp/0", "/dns4/localhost/tcp/0"];
    ...
    try {
      ...
      apiAddrs.forEach(addr =>
        expect(err.stdout).to.include(`API listening on ${addr.slice(0, -2)}`)
      );
      gatewayAddrs.forEach(addr =>
        expect(err.stdout).to.include(
          `Gateway (read only) listening on ${addr.slice(0, -2)}`
        )
      );
    }
  });
  ...
  it("should print version info", async function() {
    ...
    try {
      ...
      expect(err.stdout).to.include(
        `System version: ${os.arch()}/${os.platform()}`
      );
      expect(err.stdout).to.include(
  ...
  it("should init with custom config", async function() {
    this.timeout(100 * 1000);
    const configPath = tempWrite.sync(
      '{"Addresses": {"API": "/ip4/127.0.0.1/tcp/9999"}}',
      "config.json"
    );
