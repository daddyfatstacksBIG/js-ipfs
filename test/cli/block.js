  ...
  runOnAndOff(thing => {
    ...
    it("put with flags, format and mhtype", async function() {
      ...
      const out = await ipfs(
        "block put --format eth-block --mhtype keccak-256 test/fixtures/test-data/eth-block"
      );
      expect(out).to.eql(
        "bagiacgzarkhijr4xmbp345ovwwxra7kcecrnwcwtl7lg3g7d2ogyprdswjwq\n"
      );
    });
    ...
    it("should put and print CID encoded in specified base", async function() {
      ...
      const out = await ipfs(
        "block put test/fixtures/test-data/hello --cid-base=base64"
      );
    ...
    it("get", async function() {
      ...
      const out = await ipfs(
        "block get QmZjTnYw2TFhn9Nn7tjmPSoTBoY7YRkwPzwSrSbabY24Kp"
      );
    ...
    it("get block from file without a final newline", async function() {
      ...
      const out2 = await ipfs(
        "block get QmTwbQs4sGcCiPxV97SpbHS7QgmVg9SiKxcG1AcF1Ly2SL"
      );
    ...
    it("stat", async function() {
      ...
      const out = await ipfs(
        "block stat QmZjTnYw2TFhn9Nn7tjmPSoTBoY7YRkwPzwSrSbabY24Kp"
      );
      expect(out).to.eql(
        [
    ...
    it("should stat and print CID encoded in specified base", async function() {
      ...
      const out2 = await ipfs(
        "block stat QmZjTnYw2TFhn9Nn7tjmPSoTBoY7YRkwPzwSrSbabY24Kp --cid-base=base64"
      );
      expect(out2).to.eql(
        [
    ...
    it.skip("rm", async function() {
      ...
      const out = await ipfs(
        "block rm QmZjTnYw2TFhn9Nn7tjmPSoTBoY7YRkwPzwSrSbabY24Kp"
      );
      expect(out).to.eql(
