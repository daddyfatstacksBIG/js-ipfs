...
describe("object", () =>
  runOnAndOff(thing => {
    ...
    it("new", async () => {
      ...
      expect(out).to.eql("QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n\n");
    });

    it("new unixfs-dir", async () => {
      const out = await ipfs("object new unixfs-dir");
      expect(out).to.eql("QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn\n");
    });
    ...
    it.skip("should new and print CID encoded in specified base", async () => {
      ...
      expect(out).to.eql("mAXASIOOwxEKY/BwUmvv0yJlvuSQnrkHkZJuTTKSVmRt4UrhV\n");
    });

    it("get", async () => {
      const out = await ipfs(
        "object get QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n"
      );
    ...
    it("get with data", async function() {
      ...
      const out = await ipfs("object new");
      const out2 = await ipfs(
        `object patch set-data ${out.trim()} ${path.resolve(
          path.join(__dirname, "..")
        )}/fixtures/test-data/hello`
      );
    ...
    it("should get and print CIDs encoded in specified base", async function() {
      ...
      const out = await ipfs(
        `add ${path.resolve(
          path.join(__dirname, "..")
        )}/fixtures/planets -r --cid-version=1`
      );
    ...
    it("put", async () => {
      const out = await ipfs(
        `object put ${path.resolve(
          path.join(__dirname, "..")
        )}/fixtures/test-data/node.json`
      );
    ...
    it.skip("should put and print CID encoded in specified base", async () => {
      const out = await ipfs(
        `object put ${path.resolve(
          path.join(__dirname, "..")
        )}/fixtures/test-data/node.json --cid-base=base64`
      );
    ...
    it("stat", async function() {
      ...
      const out = await ipfs(
        "object stat QmZZmY4KCu9r3e7M2Pcn46Fc5qbn6NpzaAGaYb22kbfTqm"
      );
      expect(out).to.eql(
        [
          "NumLinks: 1",
    ...
    it("data", async () => {
      const out = await ipfs(
        "object data QmZZmY4KCu9r3e7M2Pcn46Fc5qbn6NpzaAGaYb22kbfTqm"
      );
    ...
    it("links", async () => {
      const out = await ipfs(
        "object links QmZZmY4KCu9r3e7M2Pcn46Fc5qbn6NpzaAGaYb22kbfTqm"
      );
      expect(out).to.eql(
    ...
    describe("patch", function() {
      ...
      it("append-data", async () => {
        const out = await ipfs(
          `object patch append-data QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n ${path.resolve(
            path.join(__dirname, "..")
          )}/fixtures/test-data/badconfig`
        );
        expect(out).to.eql("QmfY37rjbPCZRnhvvJuQ46htW3VCAWziVB991P79h6WSv6\n");
      });
      ...
      it.skip("should append-data and print CID encoded in specified base", async () => {
        const out = await ipfs(
          `object patch append-data QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n ${path.resolve(
            path.join(__dirname, "..")
          )}/fixtures/test-data/badconfig --cid-base=base64`
        );
        expect(out).to.eql(
      ...
      it("set-data", async () => {
        const out = await ipfs(
          `object patch set-data QmfY37rjbPCZRnhvvJuQ46htW3VCAWziVB991P79h6WSv6 ${path.resolve(
            path.join(__dirname, "..")
          )}/fixtures/test-data/badconfig`
        );
        expect(out).to.eql("QmfY37rjbPCZRnhvvJuQ46htW3VCAWziVB991P79h6WSv6\n");
      ...
      it("add-link", async () => {
        const out = await ipfs(
          "object patch add-link QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n foo QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn"
        );
        expect(out).to.eql("QmdVHE8fUD6FLNLugtNxqDFyhaCgdob372hs6BYEe75VAK\n");
      });

      // TODO: unskip after switch to v1 CIDs by default
      it.skip("should add-link and print CID encoded in specified base", async () => {
        const out = await ipfs(
          "object patch add-link QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n foo QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn --cid-base=base64"
        );
        expect(out).to.eql(
      ...
      it("rm-link", async () => {
        const out = await ipfs(
          "object patch rm-link QmdVHE8fUD6FLNLugtNxqDFyhaCgdob372hs6BYEe75VAK foo"
        );
        expect(out).to.eql("QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n\n");
      });
      ...
      it.skip("should rm-link and print CID encoded in specified base", async () => {
        const out = await ipfs(
          "object patch rm-link QmdVHE8fUD6FLNLugtNxqDFyhaCgdob372hs6BYEe75VAK foo --cid-base=base64"
        );
        expect(out).to.eql(
