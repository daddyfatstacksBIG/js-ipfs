...
describe("files", () =>
  runOnAndOff(thing => {
    ...
    const readme = fs
      .readFileSync(
        path.join(process.cwd(), "/src/init-files/init-docs/readme")
      )
      .toString("utf-8");
    ...
    it("add multiple", async function() {
      ...
      const out = await ipfs(
        "add",
        "src/init-files/init-docs/readme",
        "test/fixtures/odd-name-[v0]/odd name [v1]/hello",
        "--wrap-with-directory"
      );
    ...
    it("add recursively test", async function() {
      ...
      const out = await ipfs(
        "add -r test/fixtures/test-data/recursive-get-dir"
      );
    ...
    it("add with cid-version=0", async function() {
      ...
      const out = await ipfs(
        "add src/init-files/init-docs/readme --cid-version=0"
      );
      expect(out).to.eql(
        "added QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB readme\n"
      );
    ...
    it("add with cid-version=1 < default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/less-than-default-max-chunk-size --cid-version=1"
      );
      expect(out).to.eql(
        "added bafkreie3abdlmy7dtxddm4vkbhd2x2yax5scf4m35eyxf3ywj2uwkjbhgi less-than-default-max-chunk-size\n"
      );
    });
    ...
    it("add with cid-version=1 > default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/greater-than-default-max-chunk-size --cid-version=1"
      );
      expect(out).to.eql(
        "added bafybeiebjiahgamnil3r3os2hqorjualrgx6hpqh5zuqbhjmt7tyjqqttm greater-than-default-max-chunk-size\n"
      );
    });
    ...
    it("add with cid-version=1 and raw-leaves=false < default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/less-than-default-max-chunk-size --cid-version=1 --raw-leaves=false"
      );
      expect(out).to.eql(
        "added bafybeiaoh5okvpnuhndsz4kgdhulnkm566rz7w7il6r2zm6wxu5f5uqlsu less-than-default-max-chunk-size\n"
      );
    });
    ...
    it("add with cid-version=1 and raw-leaves=false > default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/greater-than-default-max-chunk-size --cid-version=1 --raw-leaves=false"
      );
      expect(out).to.eql(
        "added bafybeiajpcrr2qglyeq3biilzt25ty6kpjs7huy2lfxopqfke6riaaxnim greater-than-default-max-chunk-size\n"
      );
    });
    ...
    it("add with cid-version=1 and raw-leaves=true < default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/less-than-default-max-chunk-size --cid-version=1 --raw-leaves=true"
      );
      expect(out).to.eql(
        "added bafkreie3abdlmy7dtxddm4vkbhd2x2yax5scf4m35eyxf3ywj2uwkjbhgi less-than-default-max-chunk-size\n"
      );
    ...
    it("add with cid-version=1 and raw-leaves=true > default max chunk size", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/greater-than-default-max-chunk-size --cid-version=1 --raw-leaves=true"
      );
      expect(out).to.eql(
        "added bafybeiebjiahgamnil3r3os2hqorjualrgx6hpqh5zuqbhjmt7tyjqqttm greater-than-default-max-chunk-size\n"
      );
    ...
    it("add --quiet", async function() {
      ...
      expect(out).to.eql("QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB\n");
    });
    ...
    it("add --quieter", async function() {
      ...
      expect(out).to.eql("QmYRMUVULBfj7WrdPESnwnyZmtayN6Sdrwh1nKcQ9QgQeZ\n");
    });
    ...
    it("add --only-hash does not add a file to the datastore", async function() {
      ...
      await Promise.race([ipfs.fail(`object get ${hash}`), delay(4000)]);
    ...
    HASH_ALGS.forEach(name => {
      it(`add with hash=${name} and raw-leaves=false`, async function() {
        ...
        const out = await ipfs(
          `add src/init-files/init-docs/readme --hash=${name} --raw-leaves=false`
        );
        const hash = out.split(" ")[1];
    ...
    it("should add and print CID encoded in specified base", async function() {
      ...
      const out = await ipfs(
        "add test/fixtures/test-data/hello --cid-base=base64"
      );
      expect(out).to.eql(
        "added mAXASIEbUSBS5xa8UHDqqt8BdxehE6tX5HxKFiwIeukV2i0wO hello\n"
      );
    });
    ...
    it("cat", async function() {
      ...
      const out = await ipfs(
        "cat QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB"
      );
    ...
    it("cat part of a file using `count`", async function() {
      ...
      const out = await ipfs(
        "cat QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB --offset 21 --count 5"
      );
    ...
    it("cat part of a file using `length`", async function() {
      ...
      const out = await ipfs(
        "cat QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB --offset 21 --length 5"
      );
      expect(out).to.eql(readme.substring(21, 26));
    });

    it("cat non-existent file", async () => {
      const err = await ipfs.fail(
        "cat QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB/dummy"
      );
      expect(err).to.exist();
    });
    ...
    it("get", async function() {
      ...
      const out = await ipfs(
        "get QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB"
      );
      expect(out).to.eql(
        "Saving file(s) QmPZ9gcCEpqKTo6aq61g2nXGUhM4iCL3ewB6LDXZCtioEB\n"
      );
    ...
    it("get recursively", async function() {
      ...
      const outDir = path.join(
        process.cwd(),
        "Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z"
      );
      rimraf(outDir);

      const out = await ipfs(
        "get Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z"
      );
      expect(out).to.eql(
