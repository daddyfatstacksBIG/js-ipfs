...
describe("config", () =>
  runOnAndOff(thing => {
    ...
    before(() => {
      ...
      originalConfigPath = path.join(
        __dirname,
        "../fixtures/go-ipfs-repo/config"
      );
    ...
    describe("profile", function() {
      ...
      Object.keys(profiles).forEach(profile => {
        it(`applies profile '${profile}'`, async () => {
          ...
          expect(updatedConfig()).to.deep.equal(
            profiles[profile].transform(originalConfig)
          );
        });
      });
      ...
      it("--dry-run causes no change", async () => {
        ...
        expect(after.Discovery.MDNS.Enabled).to.equal(
          defaultConfig.Discovery.MDNS.Enabled
        );
