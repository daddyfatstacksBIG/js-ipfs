  ...
  runOnAndOff(thing => {
    ...
    it("prints added files", async function() {
      ...
      const out = await ipfs(
        "ls Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z"
      );
      expect(out).to.eql(
    ...
    it("adds a header, -v", async function() {
      this.timeout(20 * 1000);
      const out = await ipfs(
        "ls /ipfs/Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z -v"
      );
      expect(out).to.eql(
        "Hash                                           Size Name\n" +
          "QmamKEPmEH9RUsqRQsfNf5evZQDQPYL9KXg1ADeT7mkHkT -    blocks/\n" +
    ...
    it("follows a path, <hash>/<subdir>", async function() {
      ...
      const out = await ipfs(
        "ls /ipfs/Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z/init-docs"
      );
      expect(out).to.eql(
    ...
    it("recursively follows folders, -r", async function() {
      ...
      const out = await ipfs(
        "ls -r /ipfs/Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z/init-docs"
      );
      expect(out).to.eql(
        "QmZTR5bcpQD7cFgTorqxZDYaew1Wqgfbd2ud9QqGPAkK2V 1677 about\n" +
          "QmYCvbfNbCwFR45HiNP45rwJgvatpiW38D961L5qAhUM5Y 189  contact\n" +
    ...
    it("should ls and print CIDs encoded in specified base", async function() {
      ...
      const out = await ipfs(
        "ls Qmaj2NmcyAXT8dFmZRRytE12wpcaHADzbChKToMEjBsj5Z --cid-base=base64"
      );
      expect(out).to.eql(
