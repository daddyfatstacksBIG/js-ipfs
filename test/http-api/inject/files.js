  ...
  describe("/files", () => {
    ...
    describe("/add", () => {
      ...
      it.skip("should add data and return a base64 encoded CID", async () => {
        ...
        expect(res.statusCode).to.equal(200);
        expect(multibase.isEncoded(JSON.parse(res.result).Hash)).to.deep.equal(
          "base64"
        );
      });
      ...
      it("should add data using the trickle importer", async () => {
        ...
        expect(res.statusCode).to.equal(200);
        expect(JSON.parse(res.result).Hash).to.equal(
          "QmRJTAvvv1UNgCXxK9grf6u2pCT2ZQ2wCwsojpC1sTjkp9"
        );
      });
      ...
      it("should add data using the balanced importer", async () => {
        ...
        expect(res.statusCode).to.equal(200);
        expect(JSON.parse(res.result).Hash).to.equal(
          "Qmdudp5XvJr7KrqK6fQ7m2ACStoRxuwfovNHnY6dAAeUis"
        );
      });
    });
    ...
    describe("/ls", () => {
      it("should list directory contents and return a base64 encoded CIDs", async () => {
        const form = new FormData();
        form.append("file", Buffer.from("TEST" + Date.now()), {
          filename: "data.txt"
        });
        const headers = form.getHeaders();
