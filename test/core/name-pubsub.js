...
describe("name-pubsub", function() {
  ...
  before(async function() {
    ...
    nodes = await Promise.all([createNode(), createNode()]);

    nodeA = nodes[0].api;
    nodeB = nodes[1].api;

    const ids = await Promise.all([nodeA.id(), nodeB.id()]);
  ...
  it("should publish and then resolve correctly", async function() {
    ...
    await nodeB.name
      .resolve(idA.id)
      .then(
        () => expect.fail("should not have been able to resolve idA.id"),
  ...
  it("should handle event on publish correctly", async function() {
    ...
    await waitFor(alreadySubscribed);
    const messageKey = await promisify(peerId.createFromPubKey)(
      publishedMessage.key
    );
    const pubKeyPeerId = await promisify(peerId.createFromPubKey)(
      publishedMessageData.pubKey
    );
