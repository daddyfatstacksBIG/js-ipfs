...
describe("config", () => {
  ...
  it("should validate invalid init", () => {
    const cfgs = [{ init: 138 }, { init: { bits: "not an int" } }];

    cfgs.forEach(cfg => expect(() => config.validate(cfg)).to.throw());
  });

  it("should validate valid start", () => {
    const cfgs = [{ start: true }, { start: false }, { start: undefined }];
  ...
  it("should validate valid config", () => {
    const cfgs = [
      ...
      { config: { Addresses: { API: "/ip4/127.0.0.1/tcp/5002" } } },
      {
        config: {
          Addresses: {
            API: ["/ip4/127.0.0.1/tcp/5002", "/ip4/127.0.0.1/tcp/5003"]
          }
        }
      },
      ...
      { config: { Addresses: { Gateway: "/ip4/127.0.0.1/tcp/9090" } } },
      {
        config: {
          Addresses: {
            Gateway: ["/ip4/127.0.0.1/tcp/9090", "/ip4/127.0.0.1/tcp/9091"]
          }
        }
      },
      ...
      {
        config: {
          Swarm: { ConnMgr: { LowWater: undefined, HighWater: undefined } }
        }
      },
  ...
  it("should validate invalid libp2p", () => {
    const cfgs = [{ libp2p: "error" }, { libp2p: 138 }];
