...
describe("name", function() {
  describe("republisher", function() {
    ...
    it("should republish entries after 60 seconds", function(done) {
      ...
      setTimeout(function() {
        expect(node._ipns.republisher._republishEntries.calledOnce).to.equal(
          true
        );
        done();
      }, 60 * 1000);
    ...
    before(function(done) {
      ...
      parallel(
        ...
        (err, _nodes) => {
          ...
          parallel([cb => nodeA.id(cb), cb => nodeB.id(cb)], (err, ids) => {
            expect(err).to.not.exist();
  ...
  describe("errors", function() {
    ...
    it("should error to publish if does not receive private key", function() {
      return node._ipns.publisher.publish(null, ipfsRef).then(
        () => expect.fail("should have thrown when private key was missing"),
        err => {
          expect(err).to.exist();
    ...
    it("should error to publish if an invalid private key is received", function() {
      return node._ipns.publisher
        .publish({ bytes: "not that valid" }, ipfsRef)
        .then(
          () => expect.fail("should have thrown when private key was invalid"),
          err => {
            expect(err).to.exist();
          }
        );
    ...
    it("should error to publish if _updateOrCreateRecord fails", function() {
      const stub = sinon
        .stub(node._ipns.publisher, "_updateOrCreateRecord")
        .callsArgWith(4, "error");

      return node.name.publish(ipfsRef, { resolve: false }).then(
        () =>
          expect.fail("should have thrown when _updateOrCreateRecord fails"),
        err => {
          expect(err).to.exist();
    ...
    it("should error to publish if _putRecordToRouting receives an invalid peer id", function() {
      return node._ipns.publisher
        ._putRecordToRouting(undefined, undefined)
        .then(
          () => expect.fail("should have thrown if peer id was invalid"),
          err => {
            expect(err).to.exist();
          }
        );
    ...
    it("should error to publish if receives an invalid datastore key", function() {
      ...
      return node.name.publish(ipfsRef, { resolve: false }).then(
        () => expect.fail("should have thrown if datastore key was invalid"),
        err => {
          expect(err).to.exist();
    ...
    it("should error to publish if we receive a unexpected error getting from datastore", function() {
      const stub = sinon
        .stub(node._ipns.publisher._datastore, "get")
        .callsArgWith(1, "error-unexpected");

      return node.name.publish(ipfsRef, { resolve: false }).then(
        () =>
          expect.fail(
            "should have thrown if an unexpected error was received when getting from the datastore"
          ),
        err => {
          expect(err).to.exist();
    ...
    it("should error to publish if we receive a unexpected error putting to datastore", function() {
      const stub = sinon
        .stub(node._ipns.publisher._datastore, "put")
        .callsArgWith(2, "error-unexpected");

      return node.name.publish(ipfsRef, { resolve: false }).then(
        () =>
          expect.fail(
            "should have thrown if an unexpected error was received when putting to the datastore"
          ),
        err => {
          expect(err).to.exist();
    ...
    it("should error to resolve if the received name is not a string", function() {
      return node._ipns.resolver.resolve(false).then(
        () =>
          expect.fail(
            "should have thrown if the received name is not a string"
          ),
        err => {
          expect(err).to.exist();
    ...
    it("should error to resolve if receives an invalid ipns path", function() {
      return node._ipns.resolver.resolve("ipns/<cid>").then(
        () => expect.fail("should have thrown if the IPNS path was invalid"),
        err => {
          expect(err).to.exist();
          expect(err.code).to.equal("ERR_INVALID_NAME");
        }
    ...
    it("should publish and then fail to resolve if receive error getting from datastore", async function() {
      const stub = sinon
        .stub(node._ipns.resolver._routing, "get")
        .callsArgWith(1, "error-unexpected");

      await node.name.publish(ipfsRef, { resolve: false });

      return node.name.resolve(nodeId, { nocache: true }).then(
        () =>
          expect.fail(
            "should have thrown when an invalid response was received from the datastore"
          ),
        err => {
          expect(err).to.exist();
    ...
    it("should publish and then fail to resolve if does not find the record", async function() {
      const stub = sinon
        .stub(node._ipns.resolver._routing, "get")
        .callsArgWith(1, { code: Errors.notFoundError().code });
      ...
      return node.name.resolve(nodeId, { nocache: true }).then(
        () =>
          expect.fail(
            "should have thrown when failing to find the record after publish"
          ),
        err => {
          expect(err).to.exist();
          expect(err.code).to.equal("ERR_UNEXPECTED_ERROR_GETTING_RECORD");
    ...
    it("should publish and then fail to resolve if does not receive a buffer", async function() {
      ...
      return node.name.resolve(nodeId, { nocache: true }).then(
        () => expect.fail("should have thrown if a buffer was not recieved"),
        err => {
          expect(err).to.exist();
          expect(err.code).to.equal("ERR_UNEXPECTED_ERROR_GETTING_RECORD");
          stub.restore();
