...
describe("utils", () => {
  ...
  describe("parseIpfsPath", () => {
    it("parses path with no links", function() {
      expect(utils.parseIpfsPath(rootHash)).to.deep.equal({
        hash: rootHash,
        links: []
      });
    });
    ...
    it("parses path with links", function() {
      expect(utils.parseIpfsPath(`${rootHash}/docs/index`)).to.deep.equal({
        hash: rootHash,
        links: ["docs", "index"]
      });
    });
    ...
    it("parses path with /ipfs/ prefix", function() {
      expect(utils.parseIpfsPath(`/ipfs/${rootHash}/about`)).to.deep.equal({
        hash: rootHash,
        links: ["about"]
      });
    });
    ...
    it("parses non sha2-256 paths", function() {
      ...
      const keccak512 =
        "zB7S6ZdcqsTqvNhBpx3SbFTocRpAUHj1w9WQXQGyWBVEsLStNfaaNtsdFUQbRk4tYPZvnpGbtDN5gEH4uVzUwsFyJh9Ei";
      expect(utils.parseIpfsPath(keccak512)).to.deep.equal({
        hash: keccak512,
        links: []
      });
    });
  ...
  describe("resolvePath", function() {
    ...
    it("should error on invalid hashes", () => {
      return utils
        .resolvePath(node.object, "/ipfs/asdlkjahsdfkjahsdfd")
        .then(
    ...
    it("should error when a link doesn't exist", () => {
      return utils.resolvePath(node.object, `${aboutPath}/fusion`).then(
        () => expect.fail("should have errored"),
        err => {
