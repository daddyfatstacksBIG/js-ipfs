...
describe("preload", () => {
        ...
        parent: cb => {
          waterfall(
            [
                ...
                ipfs.object.put({ Data: Buffer.from(hat()), Links: [] }, done),
              (cid, done) =>
                ipfs.object.get(cid, (err, node) => done(err, { node, cid }))
            ],
            cb
          );
        },
  ...
  it("should preload content added with object.patch.rmLink", function(done) {
    ...
    waterfall(
      [
        ...
        (cid, cb) =>
          ipfs.object.get(cid, (err, node) => cb(err, { node, cid })),
        ({ node, cid }, cb) => {
          ipfs.object.put(
            {
  ...
  it("should preload content retrieved with files.ls", done => {
    ...
    waterfall(
      [
        cb =>
          ipfs.add({ path: `/t/${hat()}`, content: Buffer.from(hat()) }, cb),
        (res, cb) => {
  ...
  it("should preload content retrieved with files.lsPullStream", done => {
    ...
    waterfall(
      [
        ...
        cb => MockPreloadNode.clearPreloadCids(cb),
        cb => pull(ipfs.files.lsPullStream(`/ipfs/${dirCid}`), pull.onEnd(cb)),
        cb => MockPreloadNode.waitForCids(`/ipfs/${dirCid}`, cb)
      ],
      done
    );
  ...
  it("should preload content retrieved with files.readReadableStream", done => {
    ...
    waterfall(
      [
        ...
        cb => {
          ipfs.files
            .readReadableStream(`/ipfs/${fileCid}`)
            .on("data", () => {})
            .on("error", cb)
            .on("end", cb);
        },
  ...
  it("should preload content retrieved with files.readPullStream", done => {
    ...
    waterfall(
      [
        ...
        cb => MockPreloadNode.clearPreloadCids(cb),
        cb =>
          pull(ipfs.files.readPullStream(`/ipfs/${fileCid}`), pull.onEnd(cb)),
        cb => MockPreloadNode.waitForCids(`/ipfs/${fileCid}`, cb)
      ],
      done
    );
