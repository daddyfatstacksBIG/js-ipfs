...
function wire(targetNode, dialerNode, callback) {
  targetNode.id((err, identity) => {
    ...
    const targetAddr = addr
      .encapsulate(multiaddr(`/ipfs/${identity.id}`))
      .toString()
  ...
  describe("transfer a block between", () => {
    it("2 peers", function(done) {
      ...
      waterfall(
        [
          cb =>
            parallel(
              [cb => makeBlock(cb), cb => addNode(fDaemon, inProcNode, cb)],
              cb
            ),
          (res, cb) => {
            block = res[0];
    ...
    it("3 peers", function(done) {
      ...
      series(
        [
          cb =>
            parallel(_.range(6).map(i => makeBlock), (err, _blocks) => {
              expect(err).to.not.exist();
              blocks = _blocks;
              cb();
            }),
          cb =>
            addNode(fDaemon, inProcNode, (err, _ipfs) => {
