...
describe("gc", function() {
  ...
  describe("locks", function() {
    ...
    it("garbage collection should wait for pending block rm to finish", async () => {
      ...
      const cid1 = (await ipfs.block.put(Buffer.from("block to rm 1"), null))
        .cid;
      const cid2 = (await ipfs.block.put(Buffer.from("block to rm 2"), null))
        .cid;
    ...
    it("garbage collection should wait for pending pin add to finish", async () => {
      ...
      const cid1 = (await ipfs.block.put(
        Buffer.from("block to pin add 1"),
        null
      )).cid;
      const cid2 = (await ipfs.block.put(
        Buffer.from("block to pin add 2"),
        null
      )).cid;
    ...
    it("garbage collection should wait for pending pin rm to finish", async () => {
      // Add two blocks so that we can pin them
      const cid1 = (await ipfs.block.put(
        Buffer.from("block to pin rm 1"),
        null
      )).cid;
      const cid2 = (await ipfs.block.put(
        Buffer.from("block to pin rm 2"),
        null
      )).cid;
