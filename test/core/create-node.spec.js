  ...
  it("does not share identity with a simultaneously created node", function(done) {
      ...
      return new IPFS({
        ...
        config: {
          Addresses: {
            ...
            Swarm: [`/ip4/0.0.0.0/tcp/${4010 + _nodeNumber * 2}`]
          },
          Bootstrap: []
        },
        preload: { enabled: false }
    ...
    waterfall(
      [
        ...
        cb =>
          parallel(
            [cb => nodeA.once("start", cb), cb => nodeB.once("start", cb)],
            cb
          ),
        (_, cb) => parallel([cb => nodeA.id(cb), cb => nodeB.id(cb)], cb),
        ([idA, idB], cb) => {
      ...
      error => {
        parallel([cb => nodeA.stop(cb), cb => nodeB.stop(cb)], stopError => {
          parallel(
            [cb => repoA.teardown(cb), cb => repoB.teardown(cb)],
            teardownError => {
