...
describe("pin", function() {
  ...
  function expectPinned(hash, type = pinTypes.all, pinned = true) {
    ...
    return pin.pinManager.isPinnedWithType(hash, type).then(result => {
  ...
  describe("isPinnedWithType", function() {
    ...
    it("when node is pinned", function() {
      return pin
        .add(pins.solarWiki)
        .then(() =>
          pin.pinManager.isPinnedWithType(pins.solarWiki, pinTypes.all)
        )
    ...
    it("when node is not in datastore", function() {
      ...
      return pin.pinManager
        .isPinnedWithType(falseHash, pinTypes.all)
        .then(pinned => {
    ...
    it("when node is in datastore but not pinned", function() {
      return pin.rm(pins.root).then(() => expectPinned(pins.root, false));
    });

    it("when pinned recursively", function() {
      return pin.pinManager
        .isPinnedWithType(pins.root, pinTypes.recursive)
        .then(result => {
    ...
    it("when pinned indirectly", function() {
      return pin.pinManager
        .isPinnedWithType(pins.mercuryWiki, pinTypes.indirect)
        .then(result => {
          expect(result.pinned).to.eql(true);
    ...
    it("when pinned directly", function() {
      return pin.add(pins.mercuryDir, { recursive: false }).then(() => {
        return pin.pinManager
          .isPinnedWithType(pins.mercuryDir, pinTypes.direct)
          .then(result => {
            expect(result.pinned).to.eql(true);
    ...
    it("when not pinned", function() {
      return clearPins()
        .then(() =>
          pin.pinManager.isPinnedWithType(pins.mercuryDir, pinTypes.direct)
        )
  ...
  describe("add", function() {
    ...
    it("recursive pin parent of direct pin", function() {
      return pin
        .add(pins.solarWiki, { recursive: false })
  ...
  describe("ls", function() {
    ...
    describe("list pins of type", function() {
      ...
      it("all direct", function() {
        return pin.ls({ type: "direct" }).then(out =>
          expect(out).to.deep.include.members([
            {
              type: "direct",
              hash: "QmbJCNKXJqVK8CzbjpNFz2YekHwh3CSHpBA86uqYg3sJ8q"
      ...
      it("all recursive", function() {
        return pin.ls({ type: "recursive" }).then(out =>
          expect(out).to.deep.include.members([
            {
              type: "recursive",
              hash: "QmTAMavb995EHErSrKo7mB8dYkpaSJxu6ys1a6XJyB2sys"
      ...
      it("all indirect", function() {
        return pin.ls({ type: "indirect" }).then(out =>
          expect(out).to.deep.include.members([
            {
              type: "indirect",
              hash: "QmTMbkDfvHwq3Aup6Nxqn3KKw9YnoKzcZvuArAfQ9GF3QG"
      ...
      it("direct for CID", function() {
        return pin.ls(pins.mercuryDir, { type: "direct" }).then(out =>
      ...
      it("direct for path", function() {
        return pin
          .ls(`/ipfs/${pins.root}/mercury/`, { type: "direct" })
          .then(out =>
            expect(out).to.have.deep.members([
              {
                type: "direct",
      ...
      it("direct for path (no match)", function(done) {
        pin.ls(
          `/ipfs/${pins.root}/mercury/wiki.md`,
          { type: "direct" },
          (err, pinset) => {
            expect(err).to.exist();
      ...
      it("recursive for CID", function() {
        return pin.ls(pins.root, { type: "recursive" }).then(out =>
          expect(out).to.have.deep.members([
            {
              type: "recursive",
              hash: pins.root
      ...
      it("indirect for CID", function() {
        return pin.ls(pins.solarWiki, { type: "indirect" }).then(out =>
          expect(out).to.have.deep.members([
            {
    ...
    it("flushes", function() {
      return pin
        ...
        .then(() => {
          ...
          return clearPins().then(() => pin.add(pins.mercuryWiki));
        })
