...
describe("init", () => {
  ...
  it("pregenerated key is being used", async () => {
    ...
    const config = await repo.config.get();
    expect(config.Identity.PeerID).is.equal(
      "QmRsooYQasV5f5r834NSpdUtmejdQcpxXkK6qsozZWEihC"
    );
  });
  ...
  it("empty repo", done => {
    ipfs.init({ bits: 512, emptyRepo: true }, err => {
      ...
      const multihash = Buffer.from(
        "12205e7c3ce237f936c76faf625e90f7751a9f5eeb048f59873303c215e9cce87599",
        "hex"
      );
