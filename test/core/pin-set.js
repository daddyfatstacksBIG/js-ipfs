...
function createNodes(num, callback) {
  ...
  for (let i = 0; i < num; i++) {
    items.push(cb =>
      createNode(String(i), (err, res) =>
        cb(err, !err && res.cid.toBaseEncodedString())
      )
    );
  }
...
function createNode(data, links = [], callback) {
  ...
  cid(serialize(node), { cidVersion: 0 }).then(
    cid => {
      callback(null, {
        node,
        cid
    ...
    it.skip("stress test: stores items > (maxItems * defaultFanout) + 1", function(done) {
      ...
      createNodes(limit, (err, nodes) => {
        ...
        series(
          [
            cb =>
              pinSet.storeSet(nodes.slice(0, -1), (err, res) => {
                expect(err).to.not.exist();
                cb(null, res);
              }),
            cb =>
              pinSet.storeSet(nodes, (err, res) => {
                expect(err).to.not.exist();
                cb(null, res);
              })
          ],
