...
const directoryContent = {
  ...
  "nested-folder/hello.txt": loadFixture(
    "test/gateway/test-folder/nested-folder/hello.txt"
  ),
  "nested-folder/ipfs.txt": loadFixture(
    "test/gateway/test-folder/nested-folder/ipfs.txt"
  ),
  "nested-folder/nested.html": loadFixture(
    "test/gateway/test-folder/nested-folder/nested.html"
  ),
  "cat-folder/cat.jpg": loadFixture(
    "test/gateway/test-folder/cat-folder/cat.jpg"
  ),
  "utf8/cat-with-óąśśł-and-أعظم._.jpg": loadFixture(
    "test/gateway/test-folder/cat-folder/cat.jpg"
  ),
  "unsniffable-folder/hexagons-xml.svg": loadFixture(
    "test/gateway/test-folder/unsniffable-folder/hexagons-xml.svg"
  ),
  "unsniffable-folder/hexagons.svg": loadFixture(
...
describe("HTTP Gateway", function() {
  ...
  before(async () => {
    ...
    await http.api._ipfs.name.publish(
      "QmW2WQi7j6c7UgJTarActp7tDNikE4B2qXtFCfLPdsgaTQ",
      { resolve: false }
    );
  });
  ...
  it("valid CIDv0", async () => {
    ...
    expect(res.headers["content-length"])
      .to.equal(res.rawPayload.length)
      .to.equal(12);
    expect(res.headers["last-modified"]).to.equal(
      "Thu, 01 Jan 1970 00:00:01 GMT"
    );
    expect(res.headers.etag).to.equal(
      '"QmT78zSuBmuS4z925WZfrqQ1qHaJ56DQaTfyMUF7F8ff5o"'
    );
    expect(res.headers["x-ipfs-path"]).to.equal(
      "/ipfs/QmT78zSuBmuS4z925WZfrqQ1qHaJ56DQaTfyMUF7F8ff5o"
    );
    expect(res.headers.suborigin).to.equal(
      "ipfs000bafybeicg2rebjoofv4kbyovkw7af3rpiitvnl6i7ckcywaq6xjcxnc2mby"
    );
  });
  ...
  */
  ...
  it("return 304 Not Modified if client announces cached CID in If-None-Match", async () => {
    ...
    expect(resFirst.headers["cache-control"]).to.equal(
      "public, max-age=29030400, immutable"
    );
  ...
  it("return proper Content-Disposition if ?filename=foo is included in URL", async () => {
    ...
    expect(resFirst.statusCode).to.equal(200);
    expect(resFirst.headers["etag"]).to.equal(`"${cid}"`);
    expect(resFirst.headers["content-disposition"]).to.equal(
      "inline; filename*=UTF-8''pretty-name-in-utf8-%C3%B3%C3%B0%C5%9B%C3%B3%C3%B0%C5%82%C4%85%C5%9B%C5%81.txt"
    );
  });
  ...
  it("load specific byte range of a file (from-)", async () => {
    ...
    expect(resFull.headers["cache-control"]).to.equal(
      "public, max-age=29030400, immutable"
    );
    expect(resFull.headers["content-length"])
      .to.equal(resFull.rawPayload.length)
      .to.equal(fileLength);
    ...
    expect(resRange.headers["content-range"]).to.equal(
      `bytes ${range.from}-${range.length}/${fileLength}`
    );
    expect(resRange.headers["content-length"])
      .to.equal(resRange.rawPayload.length)
      .to.equal(range.length);
    ...
    expect(resRange.headers["cache-control"]).to.equal(
      "public, max-age=29030400, immutable"
    );
    expect(resRange.headers["last-modified"]).to.equal(
      "Thu, 01 Jan 1970 00:00:01 GMT"
    );
    expect(resRange.headers["content-type"]).to.equal(
  ...
  it("load specific byte range of a file (from-to)", async () => {
    ...
    expect(resRange.statusCode).to.equal(206);
    expect(resRange.headers["content-range"]).to.equal(
      `bytes ${range.from}-${range.to}/${fileLength}`
    );
    expect(resRange.headers["content-length"])
      .to.equal(resRange.rawPayload.length)
      .to.equal(range.length);
  ...
  it("load a jpg file", async () => {
    ...
    expect(res.headers.suborigin).to.equal(
      "ipfs000bafybeidsg6t7ici2osxjkukisd5inixiunqdpq2q5jy4a2ruzdf6ewsqk4"
    );
  ...
  it("load a svg file (unsniffable)", async () => {
    const hexagons =
      "QmVZoGxDvKM9KExc8gaL4uTbhdNtWhzQR7ndrY7J1gWs3F/hexagons.svg";
  ...
  it("load a svg file with xml leading declaration (unsniffable)", async () => {
    const hexagons =
      "QmVZoGxDvKM9KExc8gaL4uTbhdNtWhzQR7ndrY7J1gWs3F/hexagons-xml.svg";
  ...
  it("load a directory", async () => {
    ...
    const listedFile = res.payload.match(
      /\/ipfs\/QmW2WQi7j6c7UgJTarActp7tDNikE4B2qXtFCfLPdsgaTQ\/cat\.jpg/g
    );
  ...
  it("load a webpage index.html", async () => {
    ...
    expect(res.headers["content-length"]).to.equal(res.rawPayload.length);
    expect(res.headers.etag).to.equal(
      '"Qma6665X5k3zti8nKy7gmXK2BndNDSkgmANpV6k3FUjUeg"'
    );
    expect(res.headers.suborigin).to.equal(
      "ipfs000bafybeigccfheqv7upr4k64bkg5b5wiwelunyn2l2rbirmm43m34lcpuqqe"
    );
  ...
  it("load a webpage {hash}/nested-folder/nested.html", async () => {
    const dir =
      "QmbQD7EMEL1zeebwBsWEfA3ndgSS6F7S6iTuwuqasPgVRi/nested-folder/nested.html";
    ...
    expect(res.headers["content-length"]).to.equal(res.rawPayload.length);
    expect(res.headers.etag).to.equal(
      '"QmUBKGqJWiJYMrNed4bKsbo1nGYGmY418WCc2HgcwRvmHc"'
    );
    expect(res.headers.suborigin).to.equal(
      "ipfs000bafybeigccfheqv7upr4k64bkg5b5wiwelunyn2l2rbirmm43m34lcpuqqe"
    );
    expect(res.rawPayload).to.deep.equal(
      directoryContent["nested-folder/nested.html"]
    );
  });
  ...
  it("test(gateway): load from URI-encoded path", async () => {
    ...
    const utf8path =
      "/ipfs/QmaRdtkDark8TgXPdDczwBneadyF44JvFGbrKLTkmTUhHk/cat-with-óąśśł-and-أعظم._.jpg";
    ...
    expect(res.headers.suborigin).to.equal(
      "ipfs000bafybeiftsm4u7cn24bn2suwg3x7sldx2uplvfylsk3e4bgylyxwjdevhqm"
    );
  });
  ...
  it("load a file from IPNS", async () => {
    ...
    expect(res.headers.suborigin).to.equal(
      `ipns000${new CID(id).toV1().toBaseEncodedString("base32")}`
    );
