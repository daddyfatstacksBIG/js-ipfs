...
exports.get = {
  ...
  async handler(request, h) {
    ...
    if (Buffer.isBuffer(node.data)) {
      nodeJSON.data = node.data.toString(
        request.query["data-encoding"] || undefined
      );
    }
...
exports.stat = {
  ...
  async handler(request, h) {
    ...
    stats.Hash = cidToString(stats.Hash, {
      base: request.query["cid-base"],
      upgrade: false
    });
...
exports.patchAddLink = {
  ...
  parseArgs: (request, reply) => {
    if (
      !(request.query.arg instanceof Array) ||
  ...
  async handler(request, h) {
    ...
    try {
      ...
      cid = await ipfs.object.patch.addLink(
        root,
        new DAGLink(name, node.size, ref)
      );
      node = await ipfs.object.get(cid);
    } catch (err) {
