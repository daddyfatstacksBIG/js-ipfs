...
const getPubsubRouting = node => {
  if (!node._ipns || !node._options.EXPERIMENTAL.ipnsPubsub) {
    throw errcode(
      new Error("IPNS pubsub subsystem is not enabled"),
      "ERR_IPNS_PUBSUB_NOT_ENABLED"
    );
  }
  ...
  // Find in tiered
  const pubsub = (node._ipns.routing.stores || []).find(s =>
    IpnsPubsubDatastore.isIpnsPubsubDatastore(s)
  );

  if (!pubsub) {
    throw errcode(
      new Error("IPNS pubsub datastore not found"),
      "ERR_PUBSUB_DATASTORE_NOT_FOUND"
    );
  }
