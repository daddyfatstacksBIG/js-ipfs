...
module.exports = function config(self) {
  ...
  async function applyProfile(profileName, opts) {
    ...
    try {
      ...
      throw new Error(
        `Could not apply profile '${profileName}' to config: ${err.message}`
      );
    }
  }
};
...
const profiles = {
  ...
  lowpower: {
    description:
      "Reduces daemon overhead on the system - recommended for low power systems.",
    transform: config => {
  ...
  test: {
    description:
      "Reduces external interference of IPFS daemon - for running the daemon in test environments.",
    transform: config => {
      const defaultConfig = getDefaultConfig();
      ...
      config.Addresses.Gateway = defaultConfig.Addresses.Gateway
        ? "/ip4/127.0.0.1/tcp/0"
        : "";
      config.Addresses.Swarm = defaultConfig.Addresses.Swarm.length
        ? ["/ip4/127.0.0.1/tcp/0"]
        : [];
      config.Bootstrap = [];
      config.Discovery.MDNS.Enabled = false;
  ...
  "default-networking": {
    description:
      'Restores default network settings - inverse of "test" profile.',
    transform: config => {
      const defaultConfig = getDefaultConfig();
      ...
      config.Discovery.webRTCStar.Enabled =
        defaultConfig.Discovery.webRTCStar.Enabled;
