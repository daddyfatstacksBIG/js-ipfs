  ...
  return {
    put: callbackify.variadic(async (dagNode, options) => {
      ...
      if (options.cid && (options.format || options.hashAlg)) {
        throw new Error(
          "Can't put dag node. Please provide either `cid` OR `format` and `hashAlg` options."
        );
      } else if (
        (options.format && !options.hashAlg) ||
        (!options.format && options.hashAlg)
      ) {
        throw new Error(
          "Can't put dag node. Please provide `format` AND `hashAlg` options."
        );
      }
      ...
      if (options.version === undefined) {
        if (
          options.format === multicodec.DAG_PB &&
