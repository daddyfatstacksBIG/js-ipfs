...
module.exports = function gc(self) {
  return callbackify(async () => {
    ...
    try {
      const [blockKeys, markedSet] = await Promise.all([
...
async function createMarkedSet(ipfs) {
  ...
  await Promise.all([
    ...
    ipfs.pin.pinManager.getInternalBlocks().then(addPins),
...
async function deleteUnmarkedBlocks(ipfs, markedSet, blockKeys) {
      ...
      if (!markedSet.has(b32)) {
        ...
        queue.add(async () => {
          ...
          try {
            ...
            res.err = new Error(
              `Could not delete block with CID ${cid}: ${err.message}`
            );
          }
  ...
  log(
    `Marked set has ${markedSet.size} unique blocks. Blockstore has ${blockKeys.length} blocks. ` +
