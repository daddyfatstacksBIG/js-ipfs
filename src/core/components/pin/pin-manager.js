...
class PinManager {
  ...
  async flushPins() {
    const [dLink, rLink] = await Promise.all([
  ...
  async load() {
    ...
    const [rKeys, dKeys] = await Promise.all([
  ...
  async isPinnedWithType(multihash, type) {
    ...
    queue.addAll(
      this.recursiveKeys().map(childKey => {
  ...
  async getInternalBlocks() {
    ...
    try {
      ...
      throw new Error(
        `Could not get pin sets root from datastore: ${err.message}`
      );
    }
