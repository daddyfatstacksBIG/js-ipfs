...
class IpnsPublisher {
  ...
  async publishWithEOL(privKey, value, lifetime) {
    ...
    const record = await this._updateOrCreateRecord(
      privKey,
      value,
      lifetime,
      peerId
  ...
  async _publishEntry(key, entry) {
    ...
    try {
      ...
      log(
        `ipns record for ${key.toString("base64")} was stored in the routing`
      );

      return res;
    } catch (err) {
      const errMsg = `ipns record for ${key.toString(
        "base64"
  ...
  async _publishPublicKey(key, publicKey) {
    ...
    try {
      ...
      const errMsg = `public key for ${key.toString(
        "base64"
      )} could not be stored in the routing`;
      log.error(errMsg);
      log.error(err);
