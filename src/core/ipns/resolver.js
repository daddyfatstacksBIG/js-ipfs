  ...
  async _resolveName(name) {
    ...
    try {
      ...
      if (err.code === ERR_NOT_FOUND) {
        throw errcode(
          new Error(
            `record requested for ${name} was not found in the network`
          ),
          "ERR_NO_RECORD_FOUND"
        );
      }
      ...
      throw errcode(
        new Error(`unexpected error getting the ipns record ${peerId.id}`),
    ...
    try {
      ...
      throw errcode(
        new Error("found ipns record that we couldn't convert to a value"),
        "ERR_INVALID_RECORD_RECEIVED"
      );
    }
    ...
    try {
      ...
      if (err.code === ERR_NOT_FOUND) {
        throw errcode(
          new Error(
            `public key requested for ${name} was not found in the network`
          ),
          "ERR_NO_RECORD_FOUND"
        );
      }
      ...
      throw errcode(
        new Error(
          `unexpected error getting the public key for the ipns record ${peerId.id}`
        ),
    ...
    try {
      ...
      throw errcode(
        new Error(
          "found public key record that we couldn't convert to a value"
        ),
        "ERR_INVALID_PUB_KEY_RECEIVED"
      );
    }
