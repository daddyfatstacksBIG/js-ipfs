...
class IpnsPubsubDatastore {
  constructor(pubsub, localDatastore, peerId) {
    ...
    this._pubsubDs = new PubsubDatastore(
      pubsub,
      localDatastore,
      peerId,
      ipns.validator,
      this._handleSubscriptionKey
    );
  }
  ...
  _handleSubscriptionKey(key) {
    ...
    if (!subscriber) {
      throw errcode(
        new Error(`key ${key} does not correspond to a subscription`),
        "ERR_INVALID_KEY"
      );
    }
  ...
  async cancel(name) {
    ...
    if (typeof name !== "string") {
      throw errcode(
        new Error("invalid subscription name"),
        "ERR_INVALID_SUBSCRIPTION_NAME"
      );
    }
    ...
    const stringifiedTopic = Object.keys(this._subscriptions).find(
      key => this._subscriptions[key] === name
    );
