...
class IpnsRepublisher {
  ...
  start() {
    if (this._republishHandle) {
      throw errcode(
        new Error("republisher is already running"),
        "ERR_REPUBLISH_ALREADY_RUNNING"
      );
    }
  ...
  async stop() {
    ...
    if (!republishHandle) {
      throw errcode(
        new Error("republisher is not running"),
        "ERR_REPUBLISH_NOT_RUNNING"
      );
    }
  ...
  async _republishEntry(privateKey) {
    ...
    try {
      const peerId = await promisify(PeerId.createFromPrivKey)(
        privateKey.bytes
      );
      const value = await this._getPreviousValue(peerId);
      await this._publisher.publishWithEOL(
        privateKey,
        value,
  ...
  async _getPreviousValue(peerId) {
    ...
    try {
      ...
      if (!Buffer.isBuffer(dsVal)) {
        throw errcode(
          new Error("found ipns record that we couldn't process"),
          "ERR_INVALID_IPNS_RECORD"
        );
      }
      ...
      if (err && err.notFound) {
        throw errcode(
          new Error(`no previous entry for record with id: ${peerId.id}`),
          "ERR_NO_ENTRY_FOUND"
        );
      }
