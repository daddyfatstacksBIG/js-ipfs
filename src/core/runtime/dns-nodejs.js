...
async function recursiveResolveDnslink(domain, depth) {
  if (depth === 0) {
    throw errcode(
      new Error("recursion limit exceeded"),
      "ERR_DNSLINK_RECURSION_LIMIT"
    );
  }
...
async function resolveDnslink(domain) {
  ...
  const dnslinkRecords = flatten(records).filter(record =>
    DNSLINK_REGEX.test(record)
  );
  ...
  if (dnslinkRecords.length === 0) {
    throw errcode(
      new Error(`No dnslink records found for domain: ${domain}`),
